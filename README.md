...
> ⚠️ 说明：模板已尽可能对齐官方规范，但不保证 100% 完全一致，请在最终提交前自行核验。本人已使用该模版通过盲审，未发现任何问题。
...
- **规范适配**  
  - 基本符合 2025 版论文格式要求（页边距、字体、字号、行距、页眉双线等）
  - 符合大部分湖大参考文献格式要求（如作者不超过3个时，全部著录。超过3个时著录前3个作者，英文只有首字母大写等）
...
* 确认使用 xelatex
* 检查字体是否存在
* 检查输出文件是否被其他阅读器占用（如 main.pdf 正在被wps打开）
* 检查是否.eps图片文件未被正确转换为.pdf格式（可使用epstopdf指令将.eps转换为.pdf）

如果以上方法均无效，可点击 clean.bat 清理输出文件，然后重新编译。
如果问题依旧存在，请检查编译日志 main.aux，或者查看 LaTeX Workshop 消息，确认是否有其他错误信息。
